...
from .models import MAX_RESPONSES, Credit
...
class FreeTextResponseViewMixin(
        ...
        context.update(
            {
                ...
                "usage_id": str(self.scope_ids.usage_id).replace(":", "_").replace("@", "_").replace("/", "_").replace("+", "_").replace(".", "_"),
    ...
    def _determine_credit(self):
        #  Not a standard xblock pylint disable.
        ...
        if self.student_answer == "" or not self._word_count_valid():
            result = Credit.zero
        elif not self.fullcredit_keyphrases and not self.halfcredit_keyphrases:
            result = Credit.full
        elif _is_at_least_one_phrase_present(self.fullcredit_keyphrases, self.student_answer):
            result = Credit.full
        elif _is_at_least_one_phrase_present(self.halfcredit_keyphrases, self.student_answer):
            result = Credit.half
        else:
            result = Credit.zero
        return result
    ...
    def _get_problem_progress(self):
        ...
        if self.weight == 0:
            ...
            weight = self.weight
            temp = self.ngettext(f"{weight} point possible", f"{weight} points possible", weight)
            result = f"({temp})"
        else:
            ...
            weight = self.weight
            temp = self.ngettext(f"{score_string}/{weight} point", f"{score_string}/{weight} points", weight)
            result = f"({temp})"
        return result
    ...
    def get_other_answers(self):
        ...
        if student_answer_incorrect or shouldnt_show_other_responses:
            return []
        return_list = [response for response in self.displayable_answers if response["student_id"] != student_id]
    ...
    def _get_invalid_word_count_message(self, ignore_attempts=False):
        ...
        if (ignore_attempts or self.count_attempts > 0) and (not self._word_count_valid()):
            word_count_message = self._get_word_count_message()
            result = self.gettext("Invalid Word Count. {word_count_message}").format(
    ...
    def _generate_validation_message(self, text):
        ...
        the supplied string
        """
        result = ValidationMessage(ValidationMessage.ERROR, self.gettext(str(text)))
        return result
    ...
    def validate_field_data(self, validation, data):
        ...
        """
        if data.weight < 0:
            msg = self._generate_validation_message("Weight cannot be negative")
            validation.add(msg)
        if data.max_attempts and data.max_attempts < 0:
            msg = self._generate_validation_message("Maximum Attempts cannot be negative")
            validation.add(msg)
        if data.min_word_count < 1:
            msg = self._generate_validation_message("Minimum Word Count cannot be less than 1")
            validation.add(msg)
        if data.min_word_count > data.max_word_count:
            msg = self._generate_validation_message("Minimum Word Count cannot be greater than Max Word Count")
            validation.add(msg)
        if not data.submitted_message:
            msg = self._generate_validation_message("Submission Received Message cannot be blank")
            validation.add(msg)
